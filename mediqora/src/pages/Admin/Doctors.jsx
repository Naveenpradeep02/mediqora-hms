...
const Doctors = () => {
  // In Mediqora Super Admin portal, Super Admin can both ADD and EDIT doctors
  const canAddDoctor = true;
  ...
  const openAddModal = () => {
    ...
    setFormData({
      ...
      fee: 500,
      roomNo: `OPD Desk #${101 + doctors.length}`,
      availableDays: 'Mon - Sat',
  ...
  const handleDeleteDoctor = (doc) => {
    if (!canAddDoctor) {
      toast.error('Only Mediqora Super Admin can remove doctor profiles.');
      return;
    }
    if (doctors.length <= 1) {
      toast.error('At least 1 Senior Doctor profile must remain in the directory.');
      return;
    }
    if (window.confirm(`Are you sure you want to remove ${doc.name} from the medical directory?`)) {
      const updated = doctors.filter(d => d.id !== doc.id);
      setDoctors(updated);
      saveDoctors(updated);
      toast.success(`${doc.name} removed from medical faculty.`);
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (editingDoctor) {
      ...
      if (!canAddDoctor) {
        toast.error('Only Mediqora Super Admin can add new doctors.');
        return;
      }
      const newDoc = {
      ...
      toast.success(`New Doctor "${formData.name}" registered successfully.`);
    }
    ...
    <div className="space-y-6 font-sans pb-8">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-[#BBF7D0] shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <Stethoscope className="w-7 h-7 text-[#16a34a]" /> Senior Doctors Directory
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-1">
            Create new doctor faculty, update consultation fees, OPD desks, and availability schedules.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-extrabold text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#16a34a]"
            />
          </div>
          ...
          {canAddDoctor && (
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-[#16a34a] hover:bg-[#15803d] text-white font-extrabold text-xs shadow-md transition-all flex items-center gap-2 cursor-pointer shrink-0"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDoctors.map((doc) => (
          <div
            key={doc.id}
            className="bg-white p-6 rounded-3xl border border-[#BBF7D0] shadow-xs hover:shadow-md transition-all space-y-4 flex flex-col justify-between relative group"
          >
            <div className="space-y-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="px-2.5 py-1 rounded-full text-[10px] font-black uppercase bg-green-100 text-[#15803d] border border-[#BBF7D0] shrink-0">
                  ₹{doc.fee} OPD
                </span>
              </div>

              <div className="p-4 rounded-2xl bg-green-50/60 border border-[#BBF7D0]/60 space-y-2 text-xs font-medium">
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
              <button
                ...
                className="flex-1 py-2 px-3 rounded-xl bg-slate-900 text-emerald-400 hover:bg-slate-800 font-extrabold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-2xs"
              >
                <Edit className="w-3.5 h-3.5 text-emerald-400" /> Edit Doctor Details
              </button>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 sm:p-8 space-y-6 shadow-2xl border border-slate-200 relative max-h-[90vh] overflow-y-auto font-sans">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              <div>
                <span className="text-[10px] font-black uppercase text-[#16a34a] bg-green-50 px-2.5 py-0.5 rounded-full border border-[#BBF7D0]">
                  {editingDoctor ? 'Update Doctor Profile' : 'Register New Faculty Doctor'}
                </span>
                <h2 className="text-xl font-black text-slate-900 mt-1">
                  {editingDoctor ? `Edit ${editingDoctor.name}` : 'Create New Doctor'}
                </h2>
              </div>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs font-medium">
              ...
              <div>
                ...
                <input
                  type="email"
                  placeholder="dr.name@kkrclinic.com"
                    ...
                    <button
                      ...
                      className={`p-2 rounded-xl border text-[11px] font-bold text-center flex items-center justify-center gap-1.5 cursor-pointer transition-all ${
                        formData.badgeColor === opt.value
                          ? 'border-[#16a34a] bg-green-50 text-[#15803d] font-black'
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 rounded-xl bg-[#16a34a] hover:bg-[#15803d] text-white font-black text-xs shadow-md transition-all flex items-center gap-2 cursor-pointer"
                >
